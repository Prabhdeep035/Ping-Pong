...
class Paddle(pygame.sprite.Sprite):
    def __init__(self,groups):
            ...
            pygame.draw.rect(self.image,COLORS['paddle'],pygame.FRect((0,0),SIZE['paddle']),0,4) # for making the sides rounded
            self.rect=self.image.get_frect(topleft=POS['player'])
...
class Ball(pygame.sprite.Sprite):
    def __init__ (self,paddle_sprites,groups,update_score):
        super().__init__(groups)
        ...
        for sprite in self.paddle_sprites:
            if sprite.rect.colliderect(self.rect):
                if direction=='horizontal':
                    ...
                    if self.rect.bottom>=self.rect.top and self.old_rect.bottom<=sprite.old_rect.top:
                        self.rect.bottom=sprite.rect.top
                        self.direction.y *=-1

                    if self.rect.top<=self.rect.bottom and self.old_rect.top>=sprite.old_rect.bottom:
